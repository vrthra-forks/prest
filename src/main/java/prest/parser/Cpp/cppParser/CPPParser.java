...
import prest.parser.Cpp.contextManager.ClassScope;
import prest.parser.Cpp.contextManager.Scope;
import prest.parser.Cpp.contextManager.SymtabManager;
...
public final class CPPParser implements CPPParserConstants {
	...
	static ClassContainer col = new ClassContainer();
	static String classTrace = "";
	static String curMethod = "";
	static String curFile = "";
	static FileMetrics fileMetric = null;
	static MethodMetrics methodMetric = null;

	public ClassContainer IdentifyModules(String fileNames[]) {

		for (int i = 0; i < fileNames.length; i++) {
			try {
				ReInit(new FileInputStream(new File(fileNames[i])));
				curFile = fileNames[i];
				fileMetric = col.addPackage("global namespace")
						.addFile(curFile);
				sym.depth = 0;
				this.translation_unit();
				System.out.println("C++ source <" + fileNames[i]
						+ ">  successfully parsed for module identification.");
			} catch (ParseException e) {
				System.out.println(e.getMessage());
				System.out.println("C++ <" + fileNames[i]
	...
	static final public void func_decl_def(Scope sc) throws ParseException {
		...
		case LCURLYBRACE:
			...
			if (methodMetric != null) {
				methodMetric.setLOC(token_source.lineNo - start);
			}
			break;
		default:
			jj_la1[6] = jj_gen;
	...
	static final public void declaration() throws ParseException {
		boolean isTypedef = false;
		if (!jj_scan_token(132) || jj_2_16(2)) {
			isTypedef = declaration_specifiers();
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case LPARENTHESIS:
			case SCOPE:
	...
	static final public boolean declaration_specifiers() throws ParseException {
		...
		default:
			...
			if (jj_2_30(2147483647)) {
				...
				default:
					jj_la1[13] = jj_gen;
					if (!jj_scan_token(132) || jj_2_28(1)) {
						qualified_type();
					} else {
						jj_consume_token(-1);
						throw new ParseException();
	...
	/*
	 * void type_specifier() : {} { simple_type_specifier() | class_specifier() |
	 * enum_specifier() }
	 */
	...
	static final public void builtin_type_specifier() throws ParseException {
		...
		case BOOL:
			...
			lastQualifiedId = "bool";
			break;
		case VOID:
			jj_consume_token(VOID);
			lastQualifiedId = "void";
			break;
		case CHAR:
			jj_consume_token(CHAR);
			lastQualifiedId = "char";
			break;
		case SHORT:
			jj_consume_token(SHORT);
			lastQualifiedId = "short";
			break;
		case INT:
		...
		case LONG:
			...
			lastQualifiedId = "long";
			break;
		case FLOAT:
			jj_consume_token(FLOAT);
			lastQualifiedId = "float";
			break;
		case DOUBLE:
			jj_consume_token(DOUBLE);
			lastQualifiedId = "double";
			break;
		case SIGNED:
			jj_consume_token(SIGNED);
			lastQualifiedId = "signed";
			break;
		case UNSIGNED:
			jj_consume_token(UNSIGNED);
			lastQualifiedId = "unsigned";
			break;
		case SIZE_T:// caner: EKLEMELER
			jj_consume_token(SIZE_T);
			lastQualifiedId = "size_t";
			break;
		case TIME_T:
			jj_consume_token(TIME_T);
			lastQualifiedId = "time_t";
			break;
		case PTRDIFF_T:
			jj_consume_token(PTRDIFF_T);
			lastQualifiedId = "ptrdiff_t";
			break;
		case DWORD_PTR:
			jj_consume_token(DWORD_PTR);
			lastQualifiedId = "dword_ptr";
			break;
		case LONG_PTR:
			jj_consume_token(LONG_PTR);
			lastQualifiedId = "long_ptr";
			break;
		default:
			jj_la1[26] = jj_gen;
			jj_consume_token(-1);
	...
	static final public void class_specifier() throws ParseException {
		...
		default:
			...
			if (jj_2_39(2)) {
				...
				ClassMetrics metric = fileMetric.addClass(sym.GetCurScope()
						.getFullScopeName());
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case COLON:
					baseClasses = base_clause(sc);
					if (baseClasses != null) {
						String baseArray[] = baseClasses.split(",");
						for (int i = 0; i < baseArray.length; i++) {
							ClassMetrics jm = col.findClass(baseArray[i]);
							if (jm != null) {
								jm.increaseNumberOfchildren();
								metric.depth = (metric.depth <= jm.depth) ? (jm.depth + 1)
										: (metric.depth);

								metric.setInheritedResponseForClass(jm
										.getResponseForClass());
							}
						}
					}
				...
				jj_consume_token(RCURLYBRACE);
				sym.CloseScope();
				metric.setLOC(token_source.lineNo - start);
			} else {
				switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
				case ID:
					t = jj_consume_token(ID);
	...
	static final public String base_clause(ClassScope scope)
			throws ParseException {
		String output = null;
		String s = null;
		jj_consume_token(COLON);
		s = base_specifier(scope);
		output = s;
		label_13: while (true) {
			switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
			case COMMA:
	...
	static final public String base_specifier(ClassScope scope)
			throws ParseException {
		Token t;
		String se = null;
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
		...
		{
			if (true)
				return (se != null) ? (se + t.image) : (t.image);
		}
		throw new Error("Missing return statement in function");
	}
	...
	static final public void member_declaration() throws ParseException {
		...
		if (jj_2_41(2147483647)) {
			...
			function_definition();
			fileMetric.getClass(sym.GetCurScope().getFullScopeName())
					.increaseWeightedMethods();
	...
	static final public void member_declarator(boolean isTypedef)
			throws ParseException {
		String name;
		ClassMetrics jm = null;
		name = declarator();
		if (isTypedef)
			sym.PutTypeName(name);
		sym.GetCurScope().addSymbol(lastQualifiedId, name);
		try {
			if (isPublic)
				fileMetric.getClass(sym.GetCurScope().getFullScopeName())
						.increasePubData();
				...
				fileMetric.getClass(sym.GetCurScope().getFullScopeName())
						.increaseData();
			jm = col.findClass(lastQualifiedId);
			jm.increaseFanIn();
			fileMetric.getClass(sym.GetCurScope().getFullScopeName())
	...
	static final public Scope function_direct_declarator(boolean isTypedef)
		...
		String methodName = (name.contains("::")) ? (name.substring(name
				.lastIndexOf("::") + 2)) : (name);
		if (methodName.compareTo("operator") != 0)
			methodMetric = fileMetric.addClass(
					sym.GetCurScope().getFullScopeName()).addMethod(methodName);
		jj_consume_token(LPARENTHESIS);
		if (jj_2_69(1)) {
			parameter_list();
		} else {
	...
	static final public Scope ctor_declarator() throws ParseException {
		...
		String methodName = (name.contains("::")) ? (name.substring(name
				.lastIndexOf("::") + 2)) : (name);
		methodMetric = fileMetric
				.addClass(sym.GetCurScope().getFullScopeName()).addMethod(
						methodName);
		jj_consume_token(LPARENTHESIS);
		if (jj_2_72(2)) {
			parameter_list();
	...
	static final public void parameter_declaration_list() throws ParseException {
		...
		methodMetric.increaseFormalParameters();
		label_19: while (true) {
			if (jj_2_79(2)) {
				;
			} else {
	...
	static final public void labeled_statement() throws ParseException {
		...
		case CASE:
			...
			jj_consume_token(COLON);
			case_statement();
			// statement_list();//TODO statement(); idi
			methodMetric.addOperator("case");
			methodMetric.increaseBranchCount();
			break;
		case _DEFAULT:
			jj_consume_token(_DEFAULT);
			jj_consume_token(COLON);
			statement();
			methodMetric.addOperator("default");
			methodMetric.increaseBranchCount();
			break;
		default:
			jj_la1[86] = jj_gen;
	...
	static final public void selection_statement() throws ParseException {
		...
		case IF:
			...
			jj_consume_token(RPARENTHESIS);
			statement();
			methodMetric.addOperator("if");
			methodMetric.increaseBranchCount();
			methodMetric.increaseDecisionCount();
			methodMetric.increaseConditionCount();
			if (jj_2_96(2)) {
				jj_consume_token(ELSE);
				statement();
				methodMetric.addOperator("else");
			} else {
				;
			}
			break;
		case SWITCH:
			...
			jj_consume_token(RPARENTHESIS);
			statement();
			methodMetric.addOperator("switch");
			methodMetric.increaseDecisionCount();
			methodMetric.increaseConditionCount();
			break;
		default:
	...
	static final public void iteration_statement() throws ParseException {
		...
		case WHILE:
			...
			jj_consume_token(RPARENTHESIS);
			statement();
			methodMetric.addOperator("while");
			methodMetric.increaseBranchCount();
			methodMetric.increaseDecisionCount();
			methodMetric.increaseConditionCount();
			break;
		case DO:
			...
			jj_consume_token(RPARENTHESIS);
			jj_consume_token(SEMICOLON);
			methodMetric.addOperator("dowhile");
			methodMetric.increaseBranchCount();
			methodMetric.increaseDecisionCount();
			methodMetric.increaseConditionCount();
			break;
		case FOR:
			...
			jj_consume_token(LPARENTHESIS);
			methodMetric.addOperator("for");
			methodMetric.increaseBranchCount();
			methodMetric.increaseDecisionCount();
			methodMetric.increaseConditionCount();
			if (jj_2_97(3)) {// String s ; durumunda buraya gelmeli sanýrým
				declaration();
			} else if (jj_2_98(1)) {
				expression();
				jj_consume_token(SEMICOLON);
	...
	static final public void jump_statement() throws ParseException {
		...
		case GOTO:
			...
			jj_consume_token(ID);
			jj_consume_token(SEMICOLON);
			methodMetric.addOperator("goto");
			methodMetric.increaseBranchCount();
			methodMetric.increaseUnstructuredElement();
			break;
		case CONTINUE:
			jj_consume_token(CONTINUE);
			jj_consume_token(SEMICOLON);
			methodMetric.addOperator("continue");
			methodMetric.increaseBranchCount();
			methodMetric.increaseUnstructuredElement();
			break;
		case BREAK:
			jj_consume_token(BREAK);
			jj_consume_token(SEMICOLON);
			methodMetric.addOperator("break");
			methodMetric.increaseBranchCount();
			methodMetric.increaseUnstructuredElement();
			break;
		case RETURN:
			...
			jj_consume_token(SEMICOLON);
			methodMetric.addOperator("return");
			break;
		default:
			jj_la1[90] = jj_gen;
			jj_consume_token(-1);
	...
	static final public void handler() throws ParseException {
		...
		case CATCH:
			...
			jj_consume_token(RPARENTHESIS);
			methodMetric.addOperator("catch");
			methodMetric.increaseBranchCount();
			methodMetric.increaseUnstructuredElement();
			compound_statement();
			break;
		case 133:
			...
			if (methodMetric != null)
				methodMetric.addOperator("finally");
			compound_statement();
			break;
		default:
			jj_la1[92] = jj_gen;
	...
	static final public void throw_statement() throws ParseException {
		...
		jj_consume_token(SEMICOLON);
		if (methodMetric != null)
			methodMetric.addOperator("throw");
	}
		...
		default:
			...
			if (jj_2_113(3)) {
				...
				case SIZEOF:
					...
					methodMetric.addOperator("sizeof*");
	...
	static final public void delete_expression() throws ParseException {
		...
		if (methodMetric != null)
			methodMetric.addOperator("delete");
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
		case LSQUAREBRACKET:
			jj_consume_token(LSQUAREBRACKET);
			jj_consume_token(RSQUAREBRACKET);
	...
	static final public void unary_operator() throws ParseException {
		...
		case PLUS:
			jj_consume_token(PLUS);
			if (methodMetric != null)
				methodMetric.addOperator("+(unary)");
			break;
		case MINUS:
			jj_consume_token(MINUS);
			if (methodMetric != null)
		...
		if (jj_2_129(3)) {
			...
			label_40: while (true) {
				...
				case LPARENTHESIS:
					...
					jj_consume_token(RPARENTHESIS);
					int met = col.findMethod(s);
					if (met != -1 && methodMetric != null)
						methodMetric.addCallPair(s);
					break;
				case DOT:
	...
	static final public String id_expression() throws ParseException {
		...
		case ID:
			t = jj_consume_token(ID);
			if (methodMetric != null)
				methodMetric.addOperand(t.image);
			break;
		case OPERATOR:
			jj_consume_token(OPERATOR);
			optor();
		...
		{
			if (true)
				return (t == null) ? ("no value") : (t.image);
		}
		throw new Error("Missing return statement in function");
	}

	static final public String primary_expression() throws ParseException {
		Token t = null;
		String s = null;
		switch ((jj_ntk == -1) ? jj_ntk() : jj_ntk) {
		case THIS:
	...
	static final public void constant() throws ParseException {
		...
		case TRUETOK:
			...
			if (methodMetric != null)
				methodMetric.addOperand("true");
			break;
		case FALSETOK:
			jj_consume_token(FALSETOK);
			if (methodMetric != null)
				methodMetric.addOperand("false");
			break;
		default:
			jj_la1[121] = jj_gen;
			jj_consume_token(-1);
	...
	static final private boolean jj_3R_251_isClass() {
		if (jj_scan_token(CLASS))
			return true;
		return false;
	}

	static final private boolean jj_3R_250_isStruct() {
	...
	static final private boolean jj_3R_168() {
		Token xsp;
		xsp = jj_scanpos;
		if (jj_3R_250_isStruct()) {
			jj_scanpos = xsp;
			if (jj_scan_token(105)) {
				jj_scanpos = xsp;
				if (jj_3R_251_isClass())
					return true;
			}
		}
		xsp = jj_scanpos;
	...
	static final private boolean jj_3R_51() {// class struct falan
		Token xsp;
		xsp = jj_scanpos;
		if (jj_scan_token(98)) {
			jj_scanpos = xsp;
			if (jj_scan_token(105)) {
				...
				if (jj_scan_token(99)) {
					jj_scanpos = xsp;
					if (jj_scan_token(TYPENAME)) {// caner: hayirlisi
						return true;
					}
				}
	...
	static final private boolean jj_3R_62_isPrimitiveType() {
		Token xsp;
		xsp = jj_scanpos;
		if (jj_3R_157()) {
			jj_scanpos = xsp;
								...
								if (jj_3R_163()) {
									...
									if (jj_3R_164()) {
										...
										if (jj_3R_165()) {
											...
											if (jj_3R_166()) {
												jj_scanpos = xsp;
												if (jj_scan_token(SIZE_T)) {// caner:
													// buralar
													// benden
													jj_scanpos = xsp;
													if (jj_scan_token(TIME_T)) {
														jj_scanpos = xsp;
														if (jj_scan_token(PTRDIFF_T)) {
															jj_scanpos = xsp;
															if (jj_scan_token(DWORD_PTR)) {
																jj_scanpos = xsp;
																if (jj_scan_token(LONG_PTR)) {
																	return true;
																}
															}
														}
	...
	static final private boolean jj_3R_341() {
		if (jj_scan_token(FALSETOK))
			return true;
		return false;
	}

	static final private boolean jj_3R_340() {
		if (jj_scan_token(TRUETOK))
			return true;
		return false;
	}
	...
	static final private boolean jj_3R_336() {
		if (jj_scan_token(UNSIGNED_HEXADECIMALLONG))
			return true;
		return false;
	}

	static final private boolean jj_3R_335() {
		if (jj_scan_token(UNSIGNED_HEXADECIMALINT))
			return true;
		return false;
	}
	...
	static final private boolean jj_3R_334() {
		if (jj_scan_token(UNSIGNED_DECIMALLONG))
			return true;
		return false;
	}
	...
	static final private boolean jj_3R_333() {
		if (jj_scan_token(UNSIGNED_DECIMALINT))
			return true;
		return false;
	}

	static final private boolean jj_3R_332() {
		if (jj_scan_token(UNSIGNED_OCTALLONG))
			return true;
		return false;
	}

	static final private boolean jj_3R_331() {
		if (jj_scan_token(UNSIGNED_OCTALINT))
			return true;
		return false;
	}
	...
	static final private boolean jj_3R_130_isPlusplus() {
	...
	private static void jj_la1_0() {
		jj_la1_0 = new int[] { 0x0, 0x500000, 0x500000, 0x0, 0x1000000,
				0x400000, 0x1010000, 0x10000, 0x500000, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x400000, 0x0, 0x0, 0x400000, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x2000000, 0x10100000, 0x10100000, 0x0,
				0x800000, 0x0, 0x0, 0x800000, 0x10000, 0x0, 0x2000000, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x500000, 0x500000, 0x1000000,
				0x10000000, 0x2000000, 0x10000000, 0x0, 0x0, 0x0, 0x10000,
				0x2000000, 0x10000000, 0x400000, 0x0, 0x500000, 0x500000,
				0x40000, 0x0, 0x140000, 0x400000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x800000, 0x1810000, 0x2000000, 0x2000000, 0x8000000,
				0x10000000, 0x2000000, 0x140000, 0x40000, 0x540000, 0x140000,
				0x2000000, 0x2000000, 0x1010000, 0x0, 0x0, 0x0, 0x1000000, 0x0,
				0x0, 0x0, 0x0, 0x8000000, 0xf0000000, 0xf0000000, 0x4000000,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x440000, 0x400000, 0x40000, 0x0, 0x140000, 0x0,
				0x100000, 0x400000, 0x2000000, 0x0, 0x0, 0xf2140000, 0x2000000, };
	}

	private static void jj_la1_1() {
		jj_la1_1 = new int[] { 0x0, 0x8400800, 0x8400800, 0x0, 0x0, 0x400800,
				0x0, 0x0, 0x8400800, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x4000,
				0x0, 0x4000, 0x4000, 0x0, 0x4000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x4000, 0x0, 0x8400800, 0x8400800, 0x0, 0x0, 0x0,
				0x0, 0x400800, 0x400800, 0x0, 0x0, 0x0, 0x0, 0x400800, 0x0,
				0x8000000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x400800, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x7f, 0x7f, 0x0, 0x80, 0x100, 0x200, 0x400,
				0x3000, 0x3000, 0x3c000, 0xc0000, 0xc0000, 0x300000, 0x1c00000,
				0x80000000, 0x80000000, 0x6000000, 0x0, 0x400800, 0x0, 0x0,
				0x18700800, 0x66000000, 0x8000000, 0x0, 0x8000000, 0x0, 0x0,
				0x400800, 0x5fffffff, 0x0, };
	}

	private static void jj_la1_2() {
		jj_la1_2 = new int[] { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x4000, 0x10124042, 0x2000, 0x10124042, 0x2000, 0x10124042,
				0xc0608820, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40,
				0x10004002, 0x10004002, 0xc0608820, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x7000000, 0x0, 0x7000000,
				0x7000000, 0x0, 0x7000000, 0x0, 0x0, 0x7000000, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x40, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x100000, 0x0, 0x100000, 0x100000, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x200c0084, 0x10400, 0x108, 0x80000, 0x0, 0x10400,
				0x20040084, 0x10, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x800201,
				0x0, };
	}

	private static void jj_la1_3() {
		jj_la1_3 = new int[] { 0x20, 0x10000, 0x10000, 0x20, 0x0, 0x10000, 0x0,
				0x0, 0x10000, 0x0, 0x4902, 0x20c, 0x4902, 0x20c, 0x4902,
				0x3400, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10000, 0x4000, 0x2,
				0x102, 0x3400, 0x0, 0x0, 0x0, 0x20c, 0x0, 0x0, 0x20c, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x800, 0x800, 0x800, 0x0, 0x0, 0x10000, 0x10000,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80000, 0x0, 0x0, 0x0, 0x0,
				0x4000, 0x10000, 0x10000, 0x0, 0x80000, 0x0, 0x10000, 0x80000,
				0x0, 0x800, 0x800, 0x800, 0x20, 0x80000, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x80090, 0x8000,
				0x0, 0x10, 0x0, 0x8000, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10000, 0x40, 0xfff70000,
				0x0, 0xfff60000, 0x0, 0x0, 0x0, };
	}

	private static void jj_la1_4() {
		jj_la1_4 = new int[] { 0x0, 0x10, 0x10, 0x0, 0x0, 0x10, 0x0, 0x0, 0x10,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x10,
				0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10,
				0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10,
				0x10, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x10,
				0x0, 0x10, 0x10, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x0, 0x0, 0x20, 0x20,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
				0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x10, 0x0, 0x0, 0x0, 0x0,
				0x10, 0x8, 0x17, 0x0, 0x7, 0x0, 0x0, 0x0, };
	}
